...
private:
    ...
    /*
    inline float _partial_distance_sq_initial(unsigned n, const short *key_ranks, const float *key_values, const float *search_vector, float limit_sq)
    {
        float dsq(0);
        for (size_t i = 0; i < n && dsq < limit_sq; ++i) {
            float v = key_values[i] - search_vector[abs(key_ranks[i])];
            dsq += v * v;
        }
        return dsq;
    }*/
...
/*
    inline real_t _partial_distance_sq_initial(unsigned n, const short *key_ranks, const real_t *key_values, const real_t *search_vector, real_t limit_sq)
    {
        real_t dsq(0);
        for (size_t i = 0; i < n && dsq < limit_sq; ++i)
            dsq += _sq(key_values[i] - search_vector[abs(key_ranks[i])]);;
        return dsq;
    }
  */
    inline real_t _partial_distance_sq_initial(const search_vector_t &search, const key_t &key, real_t limit_sq)
