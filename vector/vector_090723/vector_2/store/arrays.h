...
template <typename T>             inline T sum(const T *v, const T *end)                                   { T sum(0); for (; v != end; ++v) sum += *v; return sum; }
template <typename T>             inline T mean(const T *v, const T *end)                                  { return (end - v) <= 0 ? T(0) : (sum(v, end) / (end - v)); }
template <typename T>             inline T variance(T mean, const T *v, const T *end)                      { T sum(0); for (; v != end; ++v) sum += sq(v - mean); return sum / ((end - v) - 1); }
...
template <typename T, typename U> inline T dot(const T *a, const U *b, const T *a_end)                     { T sum(0); for (; a != a_end; ++a, ++b) sum += *a * *b; return sum; }
template <typename T>             inline T dot(const T *u, const T *u_end)                                 { T sum(0); for (; u != u_end; ++u) sum += sq(*u); return sum; } // same as dot(u, u) but more efficient
template <typename T, typename U> inline T distance_sq(const T *a, const U *b, const T *a_end)             { T sum(0); for (; a != a_end; ++a, ++b) sum += sq(*a - *b); return sum; }
...
template <typename C0, typename C1> inline auto  distance_sq_limit(const C0 &a, const C1 &b, typename C0::value_type limit_sq) { return distance_sq_limit(a.data(), b.data(), _minend(a, b), limit_sq); }
template <typename C0, typename C1> inline auto  distance_limit(const C0 &a, const C1 &b, typename C0::value_type limit) { return distance_limit(a.data(), b.data(), _minend(a, b), limit); }
