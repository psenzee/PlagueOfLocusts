...
struct range_state
{
    ...
    inline range_state(const flat_vector_t &search, const container_t &store, const key_index_t &index, const const_range_t &range)
        : search(search), store(store), index(index), range(range), _minimum(std::numeric_limits<real_t>::max(), 0), distance_count(0), compare_count(0), total_top_level(0), queue(N_CLOSEST) {}

    inline range_state(const flat_vector_t &vector, const container_t &store, const key_index_t &index)
        : search(vector), store(store), index(index), _minimum(std::numeric_limits<real_t>::max(), 0), distance_count(0), compare_count(0), total_top_level(0), queue(N_CLOSEST) {}
    ...
    inline const value_index_t &minimum() const
    {
        return _minimum;
    }
    
    inline void set_minimum(const value_index_t &min)
    {
        _minimum = min;
    }
...
private:
    ...
    bool _partial_distance_less_2(const search_vector_t &search, const key_t &key, real_t limit)
    {
        ...
        const auto &sk = search.key_id.key;
        real_t limit_sq = limit * limit, minva(REAL_MAX);
        std::array<uint64_t, N / 64> bits({ 0 });
        for (size_t i = 0; i < KeyRanksN; ++i) {
            real_t val(key.values[i]), va(abs(val));
    ...
    bool _possibly_less_than_limit(const key_t &key, id_t id, real_t limit)
    {
        ...
        if (abs(sk.norm - key.norm) > limit || (BLOOM && examined.isset(id)))
            return false;
        return _partial_distance_less_3(search, key, limit);
    }
    ...
    void _update_minimum(const key_t &key, id_t id)
    {
        ...
        real_t dist_sq = ::distance_sq_limit(store.at(id), search.vector, limit_sq);
