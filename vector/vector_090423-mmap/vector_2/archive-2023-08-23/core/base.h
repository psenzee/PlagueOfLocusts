...
static constexpr size_t __mask(size_t n)   { n |= (n >> 32); n |= (n >> 16); n |= (n >> 8); n |= (n >> 4); n |= (n >> 2); return n | (n >> 1); }
static constexpr size_t _lg_plus1_uint(size_t n) { return ((n < 2) ? 1 : (1 + _lg_plus1_uint(n / 2))); }
static constexpr size_t _approx_sqrt_pow2_uint(size_t n)  { return 1 << (_lg_plus1_uint(n) >> 1); }
