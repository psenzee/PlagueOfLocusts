...
struct search_vector
{
    ...
    inline search_vector(const flat_vector_t &search) : vector(search), key_id(key_t(search), 0), ranks_sorted(key_id.key.ranks), abs_values(key_id.key.values)
    {
        std::sort(ranks_sorted.begin(), ranks_sorted.end());
        for (auto &v : abs_values) v = abs(v);
    }
