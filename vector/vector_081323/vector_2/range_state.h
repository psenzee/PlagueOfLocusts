...
struct range_state
{
    ...
    void _from_norms(const norms_t &norms, const key_id_t &sk)
    {
        ...
        //std::cout << "[final range " << (k - j) << " | " << norm - minimum.first << "-" << norm + minimum.first << "]" << std::endl;
    }
    ...
    void _from_norms_n(const key_id_t &sk)
    {
        ...
        for (size_t i = 0; i < /*KeyRanksN*/3; i++)
            _from_norms(index.norms_n(k.ranks[i]), kid);
    }

    bool _from_ranks_0(const key_id_t &sk, const const_range_t &rn, size_t rank = 0)
    {
    ...
    bool _from_ranks_1(const key_id_t &sk, const const_range_t &rn, size_t rank = 0)
    {
        size_t sz          = sk.key.ranks.size(),
               small_batch = get_small_batch_size(rank),
               limit       = std::min(sz, get_rank_limit(rank)),
               max_rank    = std::min(sz, get_max_rank_limit(rank));
        bool small = (rn.second - rn.first) < small_batch;
        if (rank >= max_rank || small) {
            if (/*found_flags || */small)
                _update_from_range(rn);
            return true;
        }
        typename key_id_t::rank_comparer c(rank);
        ...
        for (size_t p = rank, psz = std::min(rank + limit, sz); p != psz; ++p) {
            ...
            if (r.first == e) continue;
            any = false;
            // TODO we need to do different things based on the
            // size of the subrange -- perhaps have the META PARAMS at different levels
        ...
        if (rank <= 2) {
            auto i = rn.first;
            while (i < e) {
                auto ri = (*i).key.ranks[rank];
                key_id_t k(sk);
                k.key.ranks[rank] = ri;
                const_range_t r(i, std::upper_bound(i, e, k, c));
                if (!has(ri, sk.key.ranks))
                    any = _from_ranks_0(k, r, rank + 1);
                i = r.second + 1;
            }
        }
    ...
    inline size_t get_small_batch_size(size_t rank) const { return 2; } // 64 15 1 gives 99+% on 128 for all
    inline size_t get_rank_limit(size_t rank)       const { return 3; }
    inline size_t get_max_rank_limit(size_t rank)   const { return 3; }
...
private:
    ...
    void _update_minimum(const key_id_t &kid)
    {
        if (!_possibly_less_than_minimum(kid))
            return;
        if (OPTIMIZE) examined.set(kid.id);
        distance_count++;
