...
struct range_state
{
    ...
    typedef typename key_index_t::aspects_t                             dnids_t;
...
private:
    ...
    void _from_dnids(const dnids_const_range_t &range, const search_vector_t &search)
    {
        ...
        auto i = range.first, e = range.second, j = index._aspects.lower_norm(i, e, norm), k = j;
        if (j == e) return;
        /*
    ...
    void _from_dnids(const search_vector_t &search)
    {
        ...
        for (size_t i = 0; i < KeyRanksN; i++) {
            auto r = index._aspects.find_dimension(k.ranks[i]);
