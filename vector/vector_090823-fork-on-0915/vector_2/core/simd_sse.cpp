...
inline void _f32_mul_dist_sq(const float *p, const float *q, size_t chunk_index, __m128 &mul_p, __m128 &mul_q, __m128 &a, __m128 &b, __m128 &result)
{
    chunk_index *= 4;
    a = _mm_loadu_ps(p + chunk_index);
    b = _mm_loadu_ps(q + chunk_index);
    a = _mm_mul_ps(a, mul_p);
    b = _mm_mul_ps(b, mul_q);
    a = _mm_sub_ps(a, b);
    result = _f32_fmadd_ps(a, a, result);
}
...
float f32_dot__stride8_avx(const float *p, const float *q, const float * const p_end)
{
    assert((p_end - p) % 8 == 0);
    __m256 a, b, d[1];
    _f32_clear(d, 1);
...
float f32_self_dot__stride32_avx(const float *p, const float * const p_end)
{
    assert((p_end - p) % 32 == 0);
    __m256 d[4], a;
    _f32_clear(d, 4);
...
float f32_self_dot__stride8_avx(const float *p, const float * const p_end)
{
    assert((p_end - p) % 8 == 0);
    __m256 a, d[1];
    _f32_clear(d, 1);
...
float f32_normalize__stride32_avx(const float *p, float *out, const float * const p_end)
{
    double norm = sqrt(double(f32_self_dot__stride32_avx(p, p_end))),
           invn = double(1.0) / norm;
    f32_mul__avx(p, float(invn), out, p_end);
    return float(norm);
}

float f32_mul_dist_sq_limit__stride64_avx(float mul_p, float mul_q, const float *p, const float *q, const float * const p_end, float limit_sq) // about 10% faster than stride32
{
...
float f32_dist_sq__stride64_avx(const float *p, const float *q, const float * const p_end)
{
    assert((p_end - p) % 64 == 0);
    __m256 d[8], a, b;
    _f32_clear(d, 8);
...
float f32_dist_sq__stride32_avx(const float *p, const float *q, const float * const p_end)
{
    assert((p_end - p) % 32 == 0);
    __m256 d[4], a, b;
    _f32_clear(d, 4);
...
float f32_dist_sq__stride16_avx(const float *p, const float *q, const float * const p_end)
{
    assert((p_end - p) % 16 == 0);
    __m256 d[2], a, b;
    _f32_clear(d, 2);
    ...
    return _f32_accumulate(d[0], d[1]);
}

float f32_dist_sq__stride8_avx(const float *p, const float *q, const float * const p_end)
{
    assert((p_end - p) % 8 == 0);
    __m256 a, b, d[1];
    _f32_clear(d, 1);
...
float f32_dist_sq__avx(const float *p, const float *q, const float * const p_end)
{
    size_t chunk = 64, count = (p_end - p) & ~(chunk - 1);
    float result(0);
    if (count == p_end - p) return f32_dist_sq__stride64_avx(p, q, p + count);
    for (; p_end - p; p += count, q += count) {
        while ((chunk >> 1) > (p_end - p)) chunk >>= 1;
        count = (p_end - p) & ~(chunk - 1);
        float r(0);
        switch (chunk) {
        case 64: r = f32_dist_sq__stride64_avx(p, q, p + count); break;
        case 32: r = f32_dist_sq__stride32_avx(p, q, p + count); break;
        case 16: r = f32_dist_sq__stride16_avx(p, q, p + count); break;
        case  8: r = f32_dist_sq__stride8_avx (p, q, p + count); break;
        default: for (; p != p_end; ++p, ++q) r += sq(*p - *q); break;
        }
        result += r;
    }
    return result;
...
float f32_dist_sq_limit__stride64_avx(const float *p, const float *q, const float * const p_end, float limit_sq) // about 10% faster than stride32
{
    static constexpr float max = std::numeric_limits<float>::max();
    assert((p_end - p) % 64 == 0);
    __m256 d[8], a, b;
...
float f32_dist_sq_limit__stride32_avx(const float *p, const float *q, const float * const p_end, float limit_sq)
{
    static constexpr float max = std::numeric_limits<float>::max();
    assert((p_end - p) % 32 == 0);
    __m256 d[4], a, b;
...
float f32_dist_sq_limit__stride16_avx(const float *p, const float *q, const float * const p_end, float limit_sq)
{
    static constexpr float max = std::numeric_limits<float>::max();
    assert((p_end - p) % 16 == 0);
    __m256 d[2], a, b;
...
float f32_dist_sq_limit__stride8_avx(const float *p, const float *q, const float * const p_end, float limit_sq)
{
    static constexpr float max = std::numeric_limits<float>::max();
    assert((p_end - p) % 8 == 0);
    __m256 a, b, d[1];
...
float f32_dist_sq_limit__avx(const float *p, const float *q, const float * const p_end, float limit_sq)
{
    static constexpr float max = std::numeric_limits<float>::max();
    size_t chunk = 64, count = (p_end - p) & ~(chunk - 1);
    if (count == p_end - p) return f32_dist_sq_limit__stride64_avx(p, q, p_end, limit_sq);
    float result(0);
    for (; p_end - p; p += count, q += count) {
        while ((chunk >> 1) > (p_end - p)) chunk >>= 1;
        count = (p_end - p) & ~(chunk - 1);
        float r(0);
        switch (chunk) {
        case 64: r = f32_dist_sq_limit__stride64_avx(p, q, p + count, limit_sq); break;
        case 32: r = f32_dist_sq_limit__stride32_avx(p, q, p + count, limit_sq); break;
        case 16: r = f32_dist_sq_limit__stride16_avx(p, q, p + count, limit_sq); break;
        case  8: r = f32_dist_sq_limit__stride8_avx (p, q, p + count, limit_sq); break;
        default: for (; p != p_end; ++p, ++q) r += sq(*p - *q); break;
        }
        if (r == max) return r;
        result += r;
