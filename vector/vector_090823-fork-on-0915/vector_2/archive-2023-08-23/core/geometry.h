...
template <typename ContainerT0, typename ContainerT1>
inline typename ContainerT0::value_type distance_sq_limit(const ContainerT0 &a, const ContainerT1 &b, typename ContainerT0::value_type limit)
{
    typedef typename ContainerT0::value_type val_t;
    val_t sum(0);
    for (auto i = a.begin(), j = b.begin(), e = a.end(); i != e; ++i, ++j) {
        val_t d = (*i - *j); sum += d * d;
        if (sum > limit) return std::numeric_limits<val_t>::max();
    }
    return sum;
}
