...
//template <typename T> inline int     _compare_m7(T a, T b)              { return _compare_uint(_m7(a), _m7(b)); }
//template <typename T> inline int     _compare_m4(T a, T b)              { return _compare_uint(_m4(a), _m4(b)); }
...
struct key
{
    ...
    inline key(uint16_t base_id = 0) : base_id(base_id), norm(0), exp_norm(0), m_norm(0), ranks({ 0 }), values({ 0 }) {}
    inline key(const vector_t &v, uint16_t base_id = 0)  : base_id(base_id), norm(0), exp_norm(0), m_norm(0), ranks({ 0 }), values({ 0 }) { _set(v); }
    inline key(const dynamic_t &v, uint16_t base_id = 0) : base_id(base_id), norm(0), exp_norm(0), m_norm(0), ranks({ 0 }), values({ 0 }) { _set(v); }
    ...
    inline static int compare__m_norm(const self_t &a, const self_t &b)                  { return _compare_uint(a.m_norm, b.m_norm); }
    ...
    inline static int compare__exp_m_norm(const self_t &a, const self_t &b)              { int r = _compare(a.exp_norm, b.exp_norm); return r ? r : _compare_uint(a.m_norm, b.m_norm); }
    inline static int compare__ranks(const self_t &a, const self_t &b)                   { int r(0); for (size_t i = 0; i != KeyRanksN; i++) if ((r = compare__rank(i, a, b)) != 0) return r; return 0; }
    inline static int compare__base_id_type(const self_t &a, const self_t &b)            { int s = compare__base_id(a, b); return s ? s : compare__type(a, b); }
    ...
    inline static int compare__type_exp_m_norm(const self_t &a, const self_t &b)         { int s = compare__type(a, b);    return s ? s : compare__exp_m_norm(a, b); }
    ...
    inline static int compare__base_id_type_exp_m_norm(const self_t &a, const self_t &b) { int s = compare__base_id(a, b); return s ? s : compare__type_exp_m_norm(a, b); }
    inline static int compare__initial(const self_t &a, const self_t &b)                 { return compare__base_id_type_exp_m_norm(a, b); }
...
private:
    ...
    void _set(const vector_t &v)
    {
        ...
        size_t half = N / 2; // this is RanksN for the old way -- could be sqrt(N) instead; could also be KeyRanksN for faster if we don't need type/partition info
...
struct key_id
{
    ...
    struct exp_m_norm_comparer { inline bool operator()(const self_t &a, const self_t &b) const { return key_t::compare__exp_m_norm(a.key, b.key) < 0; } };
