...
struct range_state
{
    ...
    static constexpr real_t INITIAL_RATIO    = 0.85,
                            SUBSEQUENT_RATIO = 0.85;
    ...
    typedef typename key_index_t::aspects_t                             aspects_t;
    typedef typename key_t::shape_t                                     shape_t;
    typedef typename aspects_t::const_iter_t                            aspects_iter_t;
    typedef typename aspects_t::const_range_t                           aspects_range_t;
    ...
    value_index_t          kth_min;
    bloom<BLOOM_BITS>      examined;

    inline range_state(const flat_vector_t &vector, const container_t &store, const key_index_t &index)
        : search(vector), store(store), index(index), distance_count(0), compare_count(0), queue(N_CLOSEST), kth_min(value_index_t(std::numeric_limits<real_t>::max(), 0)) {}

    void update()                                           { _from_aspects(search); }

    inline const value_index_t &kth_minimum()         const { return kth_min; }
    
    inline const value_index_t &minimum()             const { return queue.best(); }
    
    inline void set_minimum(const value_index_t &min)       { queue.add(min); kth_min = queue.kth_best(); }
...
private:
    ...
    inline const key_t &_key(const aspects_iter_t &i)             const { return index._keys[(*i).id]; }

    inline bool _up(aspects_iter_t &i, const aspects_iter_t &begin, shape_t shape, real_t norm)
    {
        if (i < begin) return false;
        compare_count++;
        if ((*i).norm >= norm - kth_min.first) {
    ...
    inline bool _down(aspects_iter_t &i, const aspects_iter_t &end, shape_t shape, real_t norm)
    {
    ...
    void _from_aspects(const aspects_range_t &range, const search_vector_t &search)
    {
        auto norm = search.norm();
        auto shape = search.key_id.key.shape;
        auto i = range.first, e = range.second,
             j = index._aspects.lower_norm(i, e, norm), k = j;
        if (j == e) return;
        while (_up(j, i, shape, norm) && _down(k, e, shape, norm)) {}
        while (_up(j, i, shape, norm)) {}
        while (_down(k, e, shape, norm)) {}
    }
    
    void _from_aspects(const search_vector_t &search)
    {
        const auto &kid = search.key_id;
        const auto &k = kid.key;
        real_t base_ratio = 0.95; // first ratio
        for (size_t i = 0; i < KeyRanksN; i++) {
            auto r = index._aspects.find_dimension_shape(k.ranks[i], k.shape);
            if (r.first == r.second) continue;
            _from_aspects(r, search);
    ...
    inline real_t _partial_distance_sq_initial(const search_vector_t &search, const key_t &key, real_t limit_sq)
    {
        ...
        for (size_t i = 0; i < KeyRanksN && dsq < limit_sq; ++i)
            dsq += arrays::sq(key.values[i] - search.vector[abs(key.ranks[i])]);
        return dsq;
    }
    ...
    bool _partial_distance_less(const search_vector_t &search, const key_t &key, real_t limit)
    {
        ...
        for (size_t i = 0; i < KeyRanksN; ++i) {
            if (find_abs(sk.ranks[i], key.ranks) == -1) { // TODO here use abs_order?? -- is there a faster way here?
                ...
                dsq += arrays::sq(u - std::min(u, minva));
    ...
    void _update_minimum(const key_t &key, id_t id)
    {
        ...
        distance_count++;
        real_t limit_sq = arrays::sq(limit),
    ...
    {
        return simd::dist_sq_limit(store.at(id), search.vector, limit_sq);
