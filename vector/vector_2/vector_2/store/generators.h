...
struct real_generator
{
    ...
    const vector_t &vector(size_t count, vector_t &out) const
    {
        if (out.size() < count)
            out.resize(count);
        vector_t u(out);
        _flatish_vector(count, out);
        _hilly_vector(count, u);
        arrays::lerp(u, out, std::max(float_t(1), real_0_1()));
        u = out;
        arrays::normalize(out);
        arrays::lerp(u, out, real_0_1());
        return out;
    }*/
    ...
    const vector_t &vector(size_t count, vector_t &out) const
    {
        if (out.size() < count)
            out.resize(count);
        _hilly_vector(count, out);
        return out;
    }

private:

    const vector_t &_original_vector(size_t count, vector_t &out) const
    {
        if (out.size() < count) out.resize(count);
        for (size_t i = 0; i < count; i++) {
            float_t value = real_spread();
            int kind = fpclassify(value);
            while (kind != FP_NORMAL && kind != FP_ZERO) {
                value = real_spread();
                kind = fpclassify(value);
            }
            if (kind == FP_ZERO) value = 0.0; // no negative zeros pls
            out[i] = value;
        }
        return out;
    }

    const vector_t &_hilly_vector(size_t count, vector_t &out) const
    {
        size_t make_zeros = real_0_1() * count;
        size_t double_it  = real_0_1() * count;
        for (size_t i = 0; i < count; i++) {
            float_t value = real_spread();
            int kind = fpclassify(value);
            while (kind != FP_NORMAL && kind != FP_ZERO) {
            ...
            if (kind == FP_ZERO) value = 0.0; // no negative zeros pls
            if (make_zeros && (i % make_zeros) == 0) value = 0;
            if (double_it && (i % double_it) == 0) value *= 2.0;
            out[i] = value;
        }
        return out;
    }
        
    const vector_t &_flatish_vector(size_t count, vector_t &out) const
    {
        if (out.size() < count) out.resize(count);
        for (size_t i = 0; i < count; i++)
            out[i] = real_n1p1();
...
private:
    ...
    {
        float_t inv_length = float_t(1.0) / sqrt(arrays::dot(v));
