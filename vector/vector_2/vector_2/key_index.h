...
struct key_index
{
    ...
    static constexpr size_t SQRT_COUNT = key_t::SQRT_COUNT;

    typedef typename key_id_t::id_t                       id_t;
    typedef key_ids<container_t, KeyRanksN>               key_ids_t;
    typedef keys<container_t, KeyRanksN>                  keys_t;
    ...
    typedef aspects<real_t, COUNT, id_t, false>           aspects_t;
    typedef typename aspects_t::aspect_t                  aspect_t;
...
*/
    ...
    self_t        &index()          { _data.index(); _aspects.index(); return *this; }
...
private:
    ...
    inline void _add(const key_t &key, size_t id)
    {
        ...
        _data.add(kid);
        _keys.add(key);
        real_t sum_sq(0), base_ratio(INITIAL_RATIO);
        for (size_t i = 0, sz = key.ranks.size(); i < sz; i++) {
            _aspects.add(key.ranks[i], key.shape, key.norm, id);
            sum_sq += arrays::sq(key.values[i] / key.norm);
            real_t ratio = sqrt(sum_sq);
            if (ratio > base_ratio) break;
            base_ratio = SUBSEQUENT_RATIO;
        }
    }
};
