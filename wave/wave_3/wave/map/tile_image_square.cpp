#include "tile_image_square.h"
#include "png_file.h"

tile_image_square::tile_image_square(const size2u &tile_size, std::size_t tile_count) :
    _data(nullptr), _size(tile_size), _count(tile_count)
{
    reset(tile_size, tile_count);
}

tile_image_square::~tile_image_square()
{
    reset(size2u(0, 0), 0);
}

void tile_image_square::reset(const size2u &size, std::size_t count)
{
...
void tile_image_square::copy_tile_image(std::size_t i, uint32_t *to, std::size_t to_stride) const
{
    auto s = _size.w;
...
void tile_image_square::save_png(std::string_view filename) const
{
    write_png__rgba32(filename.data(), data(), _size.w, _size.h * _count);
}

const uint32_t *tile_image_square::_tile_from_image(const image_t &image, std::size_t i) const
{
    auto tr = image.tiles_per_row, r = i / tr, c = i % tr, tw = _size.w;
    return image.source + r * image.stride * _size.h + c * tw;
}

void tile_image_square::_copy_tile_from_image_to(const image_t &image, std::size_t i, uint32_t *to) const
{
    auto s = image.stride;
...
void tile_image_square::_copy_image_tiles(const image_t &image)
{
...
void tile_image_square::load_from_png(std::string_view filename)
{
    image_t image;
