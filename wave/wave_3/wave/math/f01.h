...
template <typename T, unsigned E> struct power_ui             { constexpr T operator()(const T &a) const { return pow_ui<E>(a); } };
template <typename T, unsigned E> struct inv_power_ui         { constexpr T operator()(const T &a) const { T v(1 - a); return 1 - pow_ui<E>(v); } };

template <typename T>             struct linear               { constexpr T operator()(const T &a) const { return a; } };
template <typename T>             struct sinusoid             { constexpr T operator()(const T &a) const { return sin(a * math::PI * 0.5); } };
template <typename T>             struct inv_sinusoid         { constexpr T operator()(const T &a) const { return (2 / math::PI) * asin(a); } };
template <typename T>             struct sine_smooth_step     { constexpr T operator()(const T &a) const { return 0.5 - cos(a * math::PI) * 0.5; } };
template <typename T>             struct s_curve_cubic        { constexpr T operator()(const T &a) const { return (a * a * (3 - 2 * a)); } };
template <typename T>             struct inv_s_curve_cubic    { constexpr T operator()(const T &a) const { return 0.5 - sin(asin(1 - 2 * a) / 3); } };
template <typename T>             struct s_curve_quintic      { constexpr T operator()(const T &a) const { T a3 = a * a * a, a4 = a3 * a, a5 = a4 * a; return (T(6) * a5) - (T(15) * a4) + (T(10) * a3); } };
template <typename T>             struct power                { T _e;     inline power(T e)            : _e(e)        {} constexpr T operator()(const T &a) const { return pow(a, _e); } };
template <typename T>             struct inv_power            { T _e;     inline inv_power(T e)        : _e(e)        {} constexpr T operator()(const T &a) const { return T(1) - pow(T(1) - a, _e); } };
template <typename T>             struct catmull_rom          { T _p, _q; inline catmull_rom(T p, T q) : _p(p), _q(q) {} constexpr T operator()(const T &a) const { return _catmull_rom_01(_p, _q, a); } };
template <typename T>             struct sigmoid              { T _e;     inline sigmoid(T e = 15)     : _e(e)        {} constexpr T operator()(const T &a) const { T d(1 + exp(-_e * (a - 0.5))); return 1 / d; } };
...
template <typename T>             struct sq                   : public power_ui<T, 2>                             {}; // { constexpr T operator()(const T &a) const { return a * a; } };
template <typename T>             struct inv_sq               : public inv_power_ui<T, 2>                         {}; // { constexpr T operator()(const T &a) const { T v(T(1) - a); return T(1) - v * v; } }
template <typename T>             struct cubed                : public power_ui<T, 3>                             {}; // { constexpr T operator()(const T &a) const { return a * a * a; } };
template <typename T>             struct inv_cubed            : public inv_power_ui<T, 3>                         {}; // { constexpr T operator()(const T &a) const { T v(T(1) - a); return T(1) - v * v * v; } }
