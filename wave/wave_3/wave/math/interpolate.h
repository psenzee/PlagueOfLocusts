...
//
template <typename T, typename U> constexpr T lerp(const T &a, const T &b, const U &t)               { return a * (U(1) - t) + b * t; }
*/
template <typename T, typename U> constexpr T linear_interpolate(const T &a, const T &b, const U &t) { return a * (U(1) - t) + b * t; }
...
//
template <typename T, typename U> constexpr T cubic_interpolate(const T &n0, const T &n1, const T &n2, const T &n3, const U &t)
{
...
// 1.0
//
template <typename T> constexpr T s_curve_cubic(const T &a) { return (a * a * (T(3) - T(2) * a)); }
template <typename T> constexpr T smooth_step(const T &a, const T &b, const T &t) { return s_curve_cubic(clamp((t - a) / (b - a))); }
...
// @a a = 1.0
//
template <typename T> constexpr T s_curve_quintic(const T &a) { T a3 = a * a * a, a4 = a3 * a, a5 = a4 * a; return (T(6) * a5) - (T(15) * a4) + (T(10) * a3); }
template <typename T> constexpr T smoother_step(const T &a, const T &b, const T &t) { return s_curve_quintic(clamp((t - a) / (b - a))); } // this actually isn't quite right, we need to rescale back from 0, 1

namespace {
// Returns binomial coefficient without explicit use of factorials,
// which can't be used with negative integers
constexpr int pascal_triangle_procedural(int a, unsigned N)
{
    int r(1);
...
template <unsigned N> constexpr int pascal_triangle   (int a) { return ((a - (N - 1)) / N) * pascal_triangle<N - 1>(a); }
template <>           constexpr int pascal_triangle<0>(int a) { return 1; }
template <>           constexpr int pascal_triangle<1>(int a) { return a; }
}
