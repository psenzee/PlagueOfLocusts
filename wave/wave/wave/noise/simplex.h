...
struct simplex
{
    ...
    {
        int3_t i(ints(p + sum(p) * F3)), j, k;
        vec3_t t(p - i + sum(i) * G3);
        ...
        auto u(t - j + G3 * 1), v(t - k + G3 * 2), w(t - 1 + G3 * 3);
