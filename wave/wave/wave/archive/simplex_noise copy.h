...
struct simplex
{
    ...
    double evaluate(const vec2_t &pin) const
    {
...
struct noise_filler
{
    ...
    inline noise_filler(double seed, const vec2_t &frequency, double amplitude)
        : _noise(seed), _frequency(frequency), _amplitude(amplitude) {}
    inline noise_filler(double seed, double frequency, double amplitude)
    : _noise(seed), _frequency({ frequency, frequency }), _amplitude(amplitude) {}
    
    inline FloatT at(const point &p) const
    {
        return at(vec2_t({ p.x, p.y }));
    }
