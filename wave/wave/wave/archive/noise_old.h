...
struct vec3
{
    ...
    inline vec3(double x = 0., double y = 0., double z = 0.) : x(x), y(y), z(z) {}
    
    inline double dot(const vec3 &v)                const { return v.x * x + v.y * y + v.z * z; }
    inline double dot(double x, double y, double z) const { return this->x * x + this->y * y + this->z * z; }
    inline double dot(double x, double y)           const { return this->x * x + this->y * y; }
};
...
struct noise
{
    ...
    double simplex2(const vec3 &pin) const
    {
        double xin = pin.x, yin = pin.y,
        ...
        i &= 255;
        j &= 255;
        const vec3 &gi0 = grad_p[i + perm[j]];
        const vec3 &gi1 = grad_p[i + i1 + perm[j + j1]];
        const vec3 &gi2 = grad_p[i + 1 + perm[j + 1]];
    ...
    double perlin2(const vec3 &pin) const
    {
        ...
        double n00 = grad_p[xi + perm[yi]].dot(x, y);
        double n01 = grad_p[xi + perm[yi + 1]].dot(x, y - 1);
        double n10 = grad_p[xi + 1 + perm[yi]].dot(x - 1, y);
        double n11 = grad_p[xi + 1 + perm[yi + 1]].dot(x - 1, y - 1);
...
struct noise_filler
{
    ...
    inline noise_filler(double seed, noise::Method method, const vec3 &prescale, double postscale)
        : _method(method), _noise(seed), _prescale(prescale), _postscale(postscale) {}
    
    inline FloatT operator()(grid_t &grid, const point &p) const
    {
        vec3 q(p.x * _prescale.x, p.y * _prescale.y);
        double value = 0;
        switch (_method) {
        case noise::PERLIN:  value = _noise.perlin2(q);  break;
        case noise::SIMPLEX: value = _noise.simplex2(q); break;
        }
        return FloatT(value * _postscale) + grid.get(p); // add it to existing noise
    }
};

const vec3 noise::grad3[] = {
