...
struct eroder
{
    ...
    eroder(grid_t &elevation, double sea_level = 0.0) :
    ...
    void commit()
    {
        base_grid_t original(_erosion);
        gaussian_smooth_3x3(_erosion);
        gaussian_smooth_3x3(_erosion);
        for (point2i p(0, 0); p.y < _size.h; p.y++) {
    ...
    double calculate_erodable_density() const
    {
        ...
        return double(erodable) / double(_size.area());
    }
};
...
struct rain_drop
{
    ...
    bool update()
    {
        ...
        std::array<double, 9> elevs = {
            _elev(point2i::NORTHWEST), _elev(point2i::NORTH),  _elev(point2i::NORTHEAST),
            _elev(point2i::WEST),      _elev(point2i::CENTER), _elev(point2i::EAST),
            _elev(point2i::SOUTHWEST), _elev(point2i::SOUTH),  _elev(point2i::SOUTHEAST)
        };
        ...
        if (len > 0) {
            ...
            _at += dir;
            at = _point();
            if (at != prev && distance(_at, _start) > 5.0) {
...
private:
    ...
    inline double _elev(point2i::Direction dir)
    {
        point2i p(_point().direction_offset(dir));
