...
template <typename T, typename Float> inline constexpr T lerp(const T &a, const T &b, Float t) { return (1 - t) * a + t * b; }
...
// todo map 0 to 1 to -1 to 1

// todo map -1 to 1 to 0 to 1

// quantize use for indexing into an array of size n
template <typename F>
inline size_t quantize(F min, F max, F val, size_t n)
{
    size_t c = n - 1;
    F t = round(norm(val, min, max) * c);
    return t < 0 ? 0 : (t > c ? c : size_t(t));
}

template <typename F> inline size_t quantize(const std::array<F, 2> &range, F val, size_t n) { return quantize(range[0], range[1], val, n); }
