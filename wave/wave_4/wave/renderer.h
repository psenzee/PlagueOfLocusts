...
struct renderer
{
    ...
    const grid::basic<double>        &_elevation;
    grid::basic<double>               _dots;
    grid::basic<vec2_t>               _gradient_field;
    double                            _sea_level;
    double                            _scale;
    vec2_t                            _sun;
    ...
    renderer(const grid::basic<double> &elevation, size_t seed = 10) :
    ...
    void save_rgba_png(const char *file) const
    {
        ...
        write_png__rgba32(file, data, sz[0], sz[1]);
        delete [] data;
    }
    ...
    void save_height_map_png(const char *file) const
    {
        ...
        write_png__grayscale16(file, data, sz[0], sz[1]);
        delete [] data;
    }
...
private:
    ...
    void _calculate_gradients()
    {
        ...
        for (point2i p { 0, 0 }; p[1] < sz[1]; p[1]++) {
            for (p[0] = 0; p[0] < sz[0]; p[0]++) {
                auto v = grid::vector3x3(_elevation, p);
    ...
    {
        size2u sz(_elevation.size());
        uint16_t *hm = new uint16_t [area(sz)];
        for (point2i p { 0, 0 }; p[1] < sz[1]; p[1]++) {
            for (p[0] = 0; p[0] < sz[0]; p[0]++) {
                hm[p[1] * sz[0] + p[0]] = uint16_t((_elevation.get(p) + 1) * 65536);
            }
        }
        return hm;
    }
    ...
    {
        size2u sz(_elevation.size());
        uint32_t *data = new uint32_t [area(sz)];
        for (point2i p { 0, 0 }; p[1] < sz[1]; p[1]++) {
            for (p[0] = 0; p[0] < sz[0]; p[0]++) {
                auto color = _get_color(p);
                data[p[1] * sz[0] + p[0]] = color_to_u32(clamp(color));
            }
        }
        return data;
    }
