...
struct world
{
    ...
    void add_voronoi()
    {
        ...
        for (point2i p { 0, 0 }; p[1] < sz[1]; p[1]++) {
            for (p[0] = 0; p[0] < sz[0]; p[0]++) {
                point2i q(p + _offset);
                vec2_t v({ double(q[0]), double(q[1]) });
    ...
    {
        return fvec3_t ({ float(p[0]), float(p[1]), std::max(float(max_a), float(max_b)) });
    }
    ...
    {
        face f;
        point2i q(int(p[0] + step.w), p[1]),
                r(int(p[0] + step.w), int(p[1] + step.h)),
                s(p[0], int(p[1] + step.h));
    ...
    void save_obj(const char *file)
    {
        ...
        fvec2_t uv_scale { 1.f / sz[0], 1.f / sz[1] };
        face_list faces;
        fvec3_t scale { 1.f, 1.f, 128.f };
        for (point2i p { 0, 0 }; p[1] < sz[1]; p[1] += step[1]) {
            for (p[0] = 0; p[0] < sz[0]; p[0] += step[0]) {
