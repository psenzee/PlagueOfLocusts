...
template <typename T>
inline T gaussian_smooth_3x3_point(const readable<T> &grid, const point2i &p)
{
...
template <typename T>
inline T gaussian_smooth_5x5_point(const readable<T> &grid, const point2i &p)
{
...
template <typename T>
void gaussian_smooth_3x3(const readable<T> &from, grid<T> &into, region area = region())
{
    area = valid_region(from, area);
    point2i b(area.min), e(area.max);
    for (point2i p(b); p[1] < e[1]; p[1]++) {
        for (p[0] = b[0]; p[0] < e[0]; p[0]++) {
            into.set(point2i { p[0] - b[0], p[1] - b[1] }, gaussian_smooth_3x3_point(from, p));
        }
    }
}

template <typename T>
void gaussian_smooth_5x5(const readable<T> &from, grid<T> &into, region area = region())
{
    area = valid_region(from, area);
    point2i b(area.min), e(area.max);
    for (point2i p(b); p[1] < e[1]; p[1]++) {
        for (p[0] = b[0]; p[0] < e[0]; p[0]++) {
            into.set(point2i { p[0] - b[0], p[1] - b[1] }, gaussian_smooth_5x5_point(from, p));
        }
    }
}

template <typename T>
void copy_region_small_to_large(const readable<T> &small, grid<T> &large, const point2i &at)
{
    size2u sz(small.size());
    for (point2i p { 0, 0 }; p[1] < sz[1]; p[1]++) {
        for (p[0] = 0; p[0] < sz[0]; p[0]++) {
            large.set(point2i { p[0] + at[0], p[1] + at[1] }, small.get(p));
        }
    }
}
