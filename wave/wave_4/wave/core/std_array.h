...
template <typename T, std::size_t N> constexpr auto max()       { return fill<T, T, N>(std::numeric_limits<T>::max()); }
template <typename T, std::size_t N> constexpr auto min()       { return fill<T, T, N>(std::numeric_limits<T>::min()); }
template <typename T, std::size_t N> constexpr auto lowest()    { return fill<T, T, N>(std::numeric_limits<T>::lowest()); }
