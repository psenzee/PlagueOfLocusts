...
inline elem *wrap(bool value)        { return value ? new symbol("T") : nullptr; }
inline elem *wrap(const char *value) { return new string_t(value); }
...
inline symbol         *head_symbol(pair *list)                           { return cast<symbol>(head(list)); }
inline elem           *head(elem *list)                                  { return head(cast<pair>(list)); }
inline pair           *head_pair(elem *list)                             { return head_pair(cast<pair>(list)); }
inline symbol         *head_symbol(elem *list)                           { return head_symbol(cast<pair>(list)); }
...
inline pair           *list(std::span<elem *> a)                         { return list(a.data(), a.size()); }
...
inline pair           *get_assoc_at(pair *p, elem *key)                  { while (p) { if (match_dot(head_pair(p), key)) return p; p = tail_pair(p); } return nullptr; }
inline elem           *get_assoc(pair *list, elem *key)                  { pair *at = get_assoc_at(list, key); return head(at); }

inline bool replace_assoc_destructive(pair *list, elem *key, elem *value)
{
...
inline pair *set_assoc_destructive(pair *list, elem *key, elem *value)
{
