...
struct keyword_node
{
    enum { SIZE = 256, BOUNDARY_CHAR = SIZE, END_SYMBOL = SIZE + 1 };

    std::array<int16_t, SIZE + 2> data;
    std::string id;
    ...
    std::ostream &print(size_t row, std::ostream &os) const
    {
        ...
        for (int i = 0; i < data.size(); i++) {
            if (data[i]) {
                switch (data[i]) {
                case BOUNDARY_CHAR: os << "\\b"; break;
                case END_SYMBOL: break;
                case 0: break;
                default: os << char(i); break;
                }
                os << ":" << data[i] << " ";
            }
        }
...
struct keyword_entry
{
    ...
    inline keyword_entry(std::string_view identifier, std::string_view symbol, bool case_sensitive = true) : identifier(identifier), symbol(symbol), case_sensitive(case_sensitive) {}
    inline keyword_entry(const keyword_entry &o) : identifier(o.identifier), symbol(o.symbol), case_sensitive(o.case_sensitive) {}
    
    inline keyword_entry &operator=(const keyword_entry &o)
    {
        if (&o != this) {
...
struct keywords
{
    ...
    std::vector<keyword_entry> keywords;
    
    void add_keyword(std::string_view s, std::string_view id, bool case_sensitive = true)
    {
        keywords.push_back(keyword_entry(s, id, case_sensitive));
    }
...
private:
    ...
    size_t _max_length() const
    {
        ...
        for (const auto &ke : keywords) {
            max = std::max(ke.identifier.size(), max);
        }
        return max;
    }
    ...
    size_t _min_length() const
    {
        size_t min = ~0ul;
        for (const auto &ke : keywords) {
            min = std::min(ke.identifier.size(), min);
        }
        return min;
    }
    ...
    void _compute(size_t max_length = 0)
    {
        ...
        for (const auto &ke : keywords) {
            if (max_length == 0 || ke.identifier.size() <= max_length) {
                _add_keyword(ke.identifier, ke.symbol, ke.case_sensitive);
            }
        }
    }
    ...
    void _add_keyword(std::string_view identifier, std::string_view symbol, bool case_sensitive)
    {
        uint16_t n = 0;
        ...
        for (auto c : identifier) {
            ...
            n = _add_row_for(n, c, case_sensitive);
            word_char = std::isalnum(c) || std::isdigit(c) || c == '_';
        }
        _add_row_for(n, !word_char ? keyword_node::END_SYMBOL : keyword_node::BOUNDARY_CHAR, case_sensitive);
        auto &kn = rows.back();
        kn.id = symbol;
    }

    int16_t _add_row_for(int n, int ch, bool case_sensitive)
    {
