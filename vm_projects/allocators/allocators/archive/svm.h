...
struct tag_type { enum Type: uint8_t { NONE = 0, INT, FLOAT, OBJECT, PAIR, FUNCN, FUNC }; };
...
struct signature
{
    ...
    typedef std::array<uint8_t, 4> types_t;
    
    enum { TYPE_NONE = 1, TYPE_INT = 2, TYPE_FLOAT = 4, TYPE_ANY = 8, TYPE_LIST = 16 };
    enum { FLAG_SIDE_EFFECTS = 1, FLAG_COMMUTATIVE = 2 };
    
    tag_type::Type  type;
    const char     *name;
    fp_t            fptr;
    function_ip     fip;
    uint8_t         in_count, out_count;
    types_t         in, out;
    uint16_t        options;
...
class signatures
{
    std::vector<signature>                   _list;
    std::unordered_map<const char *, size_t> _map;
    
    size_t _add(const signature &sig) { size_t at = _list.size(); _list.push_back(sig); return at; }
    int    _get(const char *name) { return _map.contains(name) ?  int(_map[name]) : -1;  }

public:

    size_t add(const signature &sig) { int at = _get(sig.name); if (at == -1) return _add(sig); return size_t(at); }
};
