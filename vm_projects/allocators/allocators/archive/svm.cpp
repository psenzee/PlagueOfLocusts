...
inline void _ff   (svm *v, env *e, ff_fp fn)   { v->_->setf(fn(v->_->f)); }
inline void _fff  (svm *v, env *e, fff_fp fn)  { v->_[1].setf(fn(v->_[1].f, v->_[0].f));            ++v->_; }
inline void _ffff (svm *v, env *e, ffff_fp fn) { v->_[2].setf(fn(v->_[2].f, v->_[1].f, v->_[0].f)); v->_ += 2; }
...
inline std::ostream &print_code(std::ostream &os, const code_t *ptr)
{
    os << "[";
